...
InternalNode::InternalNode( int order, Node* parent )
    : Node( order, parent )
{
...
{
    m_mappings[0].first = static_cast<InternalNode*>(parent())->key_at(index_in_parent);
...
{
    m_mappings.front().first = static_cast<InternalNode*>(parent())->key_at(parent_index);
